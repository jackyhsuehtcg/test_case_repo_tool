...
def select_team(db: Session) -> TeamDB | None:
    """
    ...
    while True:
        try:
            choice = input(f"請輸入選項編號 (1-{len(teams)}): ")
...
class TestCaseSynchronizer:
    ...
    """
    def __init__(self, db: Session, team: TeamDB, lark_client: LarkClient, lark_table_id: str):
        self.db = db
        self.team = team
    ...
    def _get_local_cases(self):
        """獲取本地資料庫的 Test Cases"""
        logger.info(f"正在從本地資料庫讀取團隊 '{self.team.name}' 的 Test Cases...")
        cases = self.db.query(TestCaseLocalDB).filter(TestCaseLocalDB.team_id == self.team.id).all()
        return {case.test_case_number: case for case in cases}

    def _get_remote_cases(self):
        """獲取遠端 Lark 表格的 Test Cases"""
        logger.info(f"正在從 Lark 表格 (ID: {self.lark_table_id}) 讀取所有紀錄...")
        try:
            self.raw_remote_records = self.lark_client.get_all_records(self.lark_table_id)
            return {
    ...
    def analyze(self):
        ...
        比對本地與遠端資料，產生同步計畫。
        """
        local_raw = self._get_local_cases()
        remote_raw = self._get_remote_cases()
    ...
    def execute_plan(self):
        ...
        try:
            ...
            if self.plan['delete_local']:
                ...
                self.db.query(TestCaseLocalDB).filter(TestCaseLocalDB.id.in_(self.plan['delete_local'])).delete(synchronize_session=False)

            if self.plan['delete_remote']:
                logger.info(f"正在刪除 {len(self.plan['delete_remote'])} 筆遠端重複或空白紀錄...")
                self.lark_client.batch_delete_records(self.lark_table_id, self.plan['delete_remote'])
            ...
            if self.plan['create_remote']:
                ...
                if records_to_create:
                    self.lark_client.batch_create_records(self.lark_table_id, records_to_create)
            ...
            if self.plan['update_remote']:
                ...
                if records_to_update:
                    self.lark_client.parallel_update_records(self.lark_table_id, records_to_update)
...
def main():
    ...
    try:
        selected_team = None
        if args.team_id:
            selected_team = db.query(TeamDB).filter(TeamDB.id == args.team_id).first()
            if not selected_team:
                logger.error(f"找不到 ID 為 {args.team_id} 的團隊。")
                return
        else:
            selected_team = select_team(db)
